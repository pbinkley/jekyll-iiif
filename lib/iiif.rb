...
class IIIF < Liquid::Tag
  def lookup(context, name)
    name == '' ? context['page']['iiif_image'] : name
  end
  ...
  def get_include(context, name)
    ...
    lib_path = File.file?(jekyll_lib_path + '/' + name + '.html') ? jekyll_lib_path : gem_lib_path
  ...
  def render_instance(image, template, context)
    ...
    # persist instancecounter in page hash - it will be
    # overwritten by each subsequent IIIF instance on page
  ...
  def render_manifest(_image, template, context)
    partial = get_include(context, template)
